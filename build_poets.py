...
A poet whose poems are all pictures — a visual poem stored as an image in the
Poems "pdf" field, where the shape on the page is the poem — brings the picture
instead, as an <img> in the same html the card already renders. Nothing is
retyped and no shape is flattened into text. A poem left as a PDF cannot be put
on a card, so those poets wait for page images and are named in the report.
...
# A visual poem lives in the Poems "pdf" File field. An image can go straight on
# the card; a PDF cannot, so it is reported rather than shipped.
IMG_EXT = re.compile(r"\.(jpe?g|png|webp|avif|gif)$", re.I)

# "Shortest poem" means nothing when every poem is a picture, so for a poet with
# more than one the card carries the poem the pull-quote on their poet page was
# taken from. That is an editorial choice and no machine can read it back out of
# a JPEG, so it is written down. Everyone else falls to the first by slug.
PICTURE_PICK = {
    "seth-leeper": "pantoublock-with-resilience-and-kerosene",
}

# The images in the CMS are whole printed pages, margins and all, so on a card
# the poem floats in the middle of its own white space. These are the same
# pages trimmed to the ink (and, where the page repeats the title the card
# already prints, trimmed below it), uploaded as site assets. The CMS keeps the
# full page — this is a card-only crop. Keyed by poem slug; a poem with no
# entry falls back to the CMS file.
CARD_IMAGE = {
    "frame-poem-rorschach-via-negativa":
        "https://cdn.prod.website-files.com/6813b8835ec760a5ec81755e/"
        "6aaa449349929e9102f892f3_frame-poem-rorschach-via-negativa-card.jpg",
    "in-the-k-shaped-economy":
        "https://cdn.prod.website-files.com/6813b8835ec760a5ec81755e/"
        "6aaa4493bbadef506a2b1b5d_in-the-k-shaped-economy-card.jpg",
    "pantoublock-with-resilience-and-kerosene":
        "https://cdn.prod.website-files.com/6813b8835ec760a5ec81755e/"
        "6aaa44935318479caa0e50bb_pantoublock-with-resilience-and-kerosene-card.jpg",
    # Nine pages of a square-set prose poem, rendered from the PDF and stacked
    # into one image. The CMS carries the same nine pages whole; this is the
    # trimmed stack, so the card is not mostly margin.
...
def build(check_avif=True):
    ...
    for p in poems:
        ...
        if not poet:
            continue
        if not body:
            asset = (fd.get("pdf") or {}).get("url")
            if asset:
                (pictures if IMG_EXT.search(asset) else pdfs).setdefault(poet, []).append(
                    {"name": fd.get("name") or "", "slug": fd.get("slug") or "", "url": asset})
            continue
    ...
    wall, no_photo, no_poem, no_image = [], [], [], []
    for poet in poets:
        ...
        if not pick and poet["id"] in pictures:
            shots = sorted(pictures[poet["id"]], key=lambda s: s["slug"])
            want = PICTURE_PICK.get(slug)
            shot = next((s for s in shots if s["slug"] == want), shots[0])
            art = CARD_IMAGE.get(shot["slug"], shot["url"])
            pick = ((0, ""), {"poem": shot["name"], "poemSlug": shot["slug"],
                              "html": '<img src="%s" alt="%s">'
                                      % (htmllib.escape(art, quote=True),
                                         htmllib.escape(shot["name"], quote=True))})
        if not pick:
            (no_image if poet["id"] in pdfs else no_poem).append(name)
            continue
    ...
    return wall, no_photo, no_poem, no_image


def report(wall, no_photo, no_poem, no_image):
    ...
    out += ["", "## Poem is a PDF (%d)" % len(no_image), "",
    ...
    out += ["- " + n for n in sorted(no_image, key=str.lower)] or ["- nobody"]
    return "\n".join(out) + "\n"
...
def main():
    ...
    try:
        wall, no_photo, no_poem, no_image = build(check_avif=not args.no_avif)
    except urllib.error.HTTPError as e:
    ...
    print("%d have no portrait, %d have a portrait but no poem, %d wait on a PDF"
          % (len(no_photo), len(no_poem), len(no_image)))
    print("%d cards carry a picture poem" % sum(1 for r in wall if r["html"].startswith("<img")))
    ...
    with open(REPORT, "w", encoding="utf-8") as f:
        f.write(report(wall, no_photo, no_poem, no_image))
